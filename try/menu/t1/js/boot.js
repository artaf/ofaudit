...
(function() {
    ...
    var debug = true //($.deparam($.param.querystring()).debug !== undefined);

    var ofaudit = window.ofaudit = window.ofaudit || {};
    _.extend(ofaudit, {
            ...
            if (jobs.length) {
                ...
                for (var k=0; k<jobs.length; k++) {
                    debug_jobs[jobs[k].name] = job = {
                        ...
                        dependents: ofaudit.__DEBUG__.get_dependents(jobs[k].name),
                        name: jobs[k].name
                    };
                    if (jobs[k].error) {
                    ...
                    var deps = ofaudit.__DEBUG__.get_dependencies( job.name );
                ...
                missing = ofaudit.__DEBUG__.get_missing_jobs();
                failed = ofaudit.__DEBUG__.get_failed_jobs();
                var unloaded = _.filter(debug_jobs, function (job) { return job.missing; });

                if (ofaudit.debug || !_.isEmpty(failed) || !_.isEmpty(unloaded)) {
                    ...
                    if (ofaudit.debug && !_.isEmpty(debug_jobs)) {
                        log('Debug:                   ', debug_jobs);
                    }
                }
            }
            ofaudit.__DEBUG__.js_modules = {
                missing: missing,
                failed: _.pluck(failed, 'name'),
            };
            ofaudit.__DEBUG__.didLogInfo.resolve();
        },
                ...
                if (!job.error) {
                    $.when(job_exec).then(
                        function (data) {
                            ...
                            def.resolve();
                            ofaudit.process_jobs(jobs, services);
                        }, function (e) {
                            job.rejected = e || true;
                            jobs.push(job);
                            def.resolve();
